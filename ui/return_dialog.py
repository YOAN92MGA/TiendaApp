...
class ReturnDialog(QDialog):
    def __init__(self, db: Session, current_user, parent=None):
        ...
        self.return_btn.clicked.connect(self.ask_password_and_return)  # <--- corregido
    ...
    def ask_password_and_return(self):
        ...
        # Primero verificar si el usuario actual tiene rol admin
        if self.current_user.role == "admin" and verify_password(password, self.current_user.password):
            admin_id = self.current_user.id
        else:
            # Buscar cualquier admin con esa contraseña
