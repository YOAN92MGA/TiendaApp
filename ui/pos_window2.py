# ui/pos_window.py - Versión moderna con grid de productos y carrito mejorado
...
class POSWindow(QWidget):
    def __init__(self, db: Session, user):
        ...
        self.setMinimumSize(1000, 750)  # Más ancho para que el carrito quepa
        self.setStyleSheet("""
            QWidget {
        ...
        splitter.setStyleSheet("QSplitter::handle { background-color: #D0D7DE; }")
        ...
        # Categorías (botones horizontales estilo píldora)
        ...
        left_layout.addLayout(self.category_layout)
        
        # Área de productos en cuadrícula (scroll)
        scroll = QScrollArea()
        ...
        left_layout.addWidget(refresh_btn, alignment=Qt.AlignLeft)
        ...
        cart_title.setStyleSheet("font-size: 16px; font-weight: bold; color: #2C3E50;")
        cart_header.addWidget(cart_title)
        self.item_count_label = QLabel("0 items | 0 unidades")
        self.item_count_label.setStyleSheet("color: #7F8C8D;")
        ...
        total_label_text.setStyleSheet("font-size: 16px; font-weight: bold;")
        ...
        self.clear_cart_btn.setStyleSheet("background-color: #E74C3C;")
        self.pay_btn = QPushButton("💰 Pagar (Ctrl+P)")
        self.pay_btn.setStyleSheet("background-color: #27AE60; font-size: 14px; padding: 10px;")
        ...
        self.daily_label.setStyleSheet("color: #7F8C8D;")
        self.monthly_label.setStyleSheet("color: #7F8C8D;")
        ...
        splitter.setSizes([800, 500])  # Izquierda más ancha para que los productos se vean bien
    ...
    def display_products_grid(self, products):
        ...
        row, col = 0, 0
        for p in products:
            # Botón con estilo moderno (tarjeta)
            btn = QPushButton()
            ...
            """)
            # Texto formateado
            text = f"""<b>{p['name']}</b><br>
<span style="color:#2C3E50;">Precio: {p['price']:.2f} CUP</span><br>
<span style="color:#7F8C8D;">Stock: {p['quantity']}</span>"""
            btn.setText(text)
            btn.clicked.connect(lambda checked, prod=p: self.show_quantity_dialog(prod))
    ...
    def update_cart_table(self):
        ...
        for row, item in enumerate(self.cart):
            ...
            del_btn = QPushButton("❌")
            del_btn.setStyleSheet("background-color: #E74C3C; border-radius: 4px; padding: 4px;")
            del_btn.clicked.connect(lambda _, r=row: self.remove_from_cart(r))
            self.cart_table.setCellWidget(row, 4, del_btn)
            edit_btn = QPushButton("✏️")
            edit_btn.setStyleSheet("background-color: #F39C12; border-radius: 4px; padding: 4px;")
    ...
    def update_total(self):
        ...
        self.total_label.setText(f"{total:.2f} CUP")
